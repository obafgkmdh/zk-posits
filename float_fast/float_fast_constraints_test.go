...
func TestFloatCircuitConstraints(t *testing.T) {
	...
	for _, param := range params {
		for _, size := range []uint{8, 12, 16} {
			...
			for i, op := range ops {
				...
				err := os.WriteFile(filepath.Join(basepath, fmt.Sprintf("../benchmarks/float_fast/%s/constraints_%s (T_RC = %d).csv", strings.ToLower(param.name), strings.ToLower(op), size)), []byte(result_op.String()), 0644)
				if err != nil {
					t.Fatal(err)
				}
			}
	...
	err := os.WriteFile(filepath.Join(basepath, "../benchmarks/float_fast/constraints.csv"), []byte(result_all.String()), 0644)
	if err != nil {
		t.Fatal(err)
	}
}
